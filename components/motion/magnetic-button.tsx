...
export default function MagneticButton({
  ...
  return (
    <m.a
      ...
      whileHover={reducedMotion ? undefined : { y: -3, scale: 1.02, boxShadow: variant === 'primary' ? '0 22px 55px rgba(255, 255, 255, 0.15)' : '0 18px 45px rgba(255, 255, 255, 0.08)' }}
      whileTap={reducedMotion ? undefined : { scale: 0.98 }}
      transition={{ duration: 0.5, ease: premiumEase }}
    >
      <span className="absolute inset-x-4 bottom-0 h-px bg-gradient-to-r from-transparent via-white/45 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      <span className="absolute inset-0 bg-white/[0.06] opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
